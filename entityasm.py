...
import unidecode
import numpy as np
from util import short_str, dameraulevenshtein, get_deletes_list, load_entities_dict
...
class EntityDisamb:
    ...
    def save(self, path):
        dill.dump(self, file(path, "wb"))

    @staticmethod
    def load(path):
        return dill.load(file(path, "rb"))
...
class EntityASM:
    ...
    def get_suggestions(self, string, id=True, silent=True, match_all_words=False):
        ...
        results = sorted(results.items(), key=lambda (ent, (matches, dist, nwords)): (
        -len(matches), dist + 1.5 * float(len_query - len(matches)) / len_query))
    ...
    def get_word_suggestions(self, string, silent=True):
        ...
        outlist = sorted(as_list, key=lambda (term, (freq, dist, ents, w)): (dist, -freq))
...
if __name__ == "__main__":
    ...
    parser.add_argument("input", type=str, default=None, help="path to input txt file")
    parser.add_argument("-med", "--max-edit-distance", type=int, default=3, help="maximum edit distance")
    parser.add_argument("-v", "--verbose", dest="verbose", action="store_true", help="verbose")
    parser.add_argument("-k", "--k", type=int, default=5, help="number of suggestions to be displayed")
    parser.set_defaults(verbose=False)

    args = parser.parse_args()

    asm = EntityASM(args.max_edit_distance, args.k, args.verbose)
    ...
    while True:
        ...
        start_time = time.time()
        print(word_in)
        print asm.get_suggestions(word_in, id=False)
        run_time = time.time() - start_time
        print '-----'
